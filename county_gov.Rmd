...
census_county_2020 <- rio::import(here::here("data", "co-est2022-alldata.csv")) %>% 
  select(STATE, COUNTY, STNAME, CTYNAME, ESTIMATESBASE2020) %>% 
  filter(COUNTY != 0,
         CTYNAME != "District of Columbia")

census_county_2020 %>% 
  filter(str_detect(STNAME, "Louisiana"))
```
...
all_counties <- pop_county %>% select(state.name, id, county, population) %>% distinct() %>% filter(str_detect(county, "county"))
```
...
```{r}
  ...
  ##
Prop1 <- c(nrow(acfrs_counties), nrow(all_counties) - nrow(acfrs_counties))

 pie(Prop1, labels = c(paste("Counties already \nin Acfrs database:", nrow(acfrs_counties)),
                      paste("Counties being processed:", nrow(all_counties) - nrow(acfrs_counties))),
     main = "Total # of counties: 3041")

```

```{r}
# 
all_counties  %>% #filter(!str_detect(county, "\\("))
  filter(!FIPS %in% acfrs_counties$FIPS) %>%  
  arrange(state.name, population) #%>%  write.csv("missing_counties.csv") 

#used the missing_counties.csv above to manually track every county. Got this list
rio::import(here::here("data", "Missing_counties_manual_checked.xlsx")) %>% #filter(is.na(notes)) 
 filter(note == "")

Prop2 <- c(170, 117, 294)

 pie(Prop2, labels = c("Can't find: 170", "Small counties, Haven't searched: 117", "\nUploaded files\nBeing discussed\n No county government: 294"),
     main = "Total # of counties being processed: 581")
```
