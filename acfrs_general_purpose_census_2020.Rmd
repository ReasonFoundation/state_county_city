...
state_gov_2020 %>% write.csv("state_gov_2020.csv")
```
...
acfrs_city_place_pop_20 %>% write.csv("acfrs_city_place_pop_20.csv")
...
acfrs_NOT_cityplace <- acfrs_general_purpose %>% # total 12,231
  filter(!id %in% acfrs_city_place_pop$id) %>%  # 5121 obs, excludes 7110 in city list above
rename(state.abb = state)
```
...
county_pop_census_acfrs_20 %>% write.csv("county_pop_census_acfrs_20.csv")
```
...
acfrs_geoID_NO_population_20 <- acfrs_NOT_cityplace %>% 
  ...
  acfrs_geoID_NO_population_20 %>% write.csv("acfrs_geoID_NO_population_20.csv")

  
 other_entities <-  acfrs_NOT_cityplace %>% 
  filter(!id %in% county_pop_census_acfrs_20$id) %>%
    filter(!id %in% acfrs_geoID_NO_population$id) 
```
