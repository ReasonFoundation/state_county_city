...
# county_3years %>% select(state.abb, name, year, population) %>% 
#   group_by(year) %>% 
#   mutate(pop_accounted = sum(population)) %>% select(year, pop_accounted) %>% distinct() %>% 
#   mutate(percent = round((pop_accounted/332031554)*100,1))
...
acfrs_top100county_20 <- place_division_gov_20 %>% 
  ...
  # being processed
  add_row(state.abb = "MA", name = "norfolk county", id = 116575, net_pension_liability = 0, population = 725999, year = 2020) %>% 
...
acfrs_top100county_21 <- place_division_gov_21 %>% 
  ...
  add_row(state.abb = "IL", name = "will county", id = 32790, net_pension_liability = 45000000, population = 697985, year = 2021) %>% select(-c(name_census, government_id)) %>% 
...
place_division_3years %>% write.csv("place_division_3years.csv")
```
...
city_gov_3years %>% write.csv("city_gov_3years.csv")
...
city_top100_3years %>% write.csv("top100_city_3years.csv")
