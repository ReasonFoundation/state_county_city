...
* Entity names in Acfrs database are not obvious, i.e., does not always contain a certain word to indicate it's government type (city, town, county). Even some names contains a certain word such as "county", it actually a town. For example: "halifax county" in acfrs is in fact a town (population 1000) of Halifax county (pop more than 30k)
...
census_all_2020 <- rio::import(here::here("data", "sub-est2022.csv")) %>% #key to variables: SUB-EST2022_dictionary.pdf
   clean_names() %>% 
...
### States from ACFRS 2021

```{r}
acfrs_state_2021 <- acfrs_general_purpose_21 %>% filter(str_detect(name, "(state of)|(district of columbia)|(commonwealth)")) %>% 
...
```{r}
state_gov_2021 <- acfrs_state_2021 %>% left_join(census_state_2020) %>% 
...
Don't use the above file "sub-est2022.csv" because that file does not have geoID

<!-- ## Census cities and places -->
<!-- ```{r} -->
<!-- census_city_2020 <- census_all_2020 %>%  -->
...
acfrs_city_place_pop_21 <- acfrs_general_purpose_21 %>% 
  filter(!id %in% state_gov_2021$id) %>% # take out state entities 
...
acfrs_city_place_pop_21 %>% write.csv("output/city_place_gov_21.csv")
```
...
acfrs_NOT_cityplace_21 <- acfrs_general_purpose_21 %>% # total 12,784
  filter(!id %in% acfrs_city_place_pop_21$id) %>%  # 5560 obs, excludes 7224 in city list above
rename(state.abb = state)
```
...
census_county_2020 <-  census_all_2020 %>% 
   filter(sumlev == 050) %>% # county level, see: FunctionalStatusCodes_census.pdf
...
```{r}
acfrs_county_21 <- acfrs_NOT_cityplace_21 %>% 
...
```{r}
louisiana_parish_21 <- acfrs_NOT_cityplace_21 %>% 
  filter(state.abb == "LA") %>% 
...
```{r testing}
# get equivalent counties form census = 30
alaska_county_census <- census_county_2020 %>% filter(state.abb == "AK") %>% 
  #change some names to match with acfrs names
  
  mutate(name = case_when(name ==  "skagway municipality" ~ "skagway",
                         name == "yakutat city and borough" ~ "yakutat borough",
                         TRUE ~ name))
  

# Alaska counties in Acfrs:
alaska_county_acfrs_21 <- acfrs_NOT_cityplace_21 %>% filter(state.abb == "AK") %>% 
  filter(!name %in% ("state of alaska")) %>% 
  filter(!str_detect(name, "(trib)|(village)"))

"anchorage municipality", "juneau city and borough","sitka city and borough", "skagway municipality", "wrangell city and borough", "yakutat city and borough"

#NOTE: Both city and borough:
#"sitka city and borough", "wrangell city and borough", "yakutat city and borough"

# join to get population into acfrs: 

alaska_county_acfrs_census_21 <- alaska_county_acfrs_21 %>% left_join(alaska_county_census)
```

### Step 3: Binding counties, parish, borough together
```{r}
# first, join entities in ACFRs contain words "County" + Louisiana that contain word "Parish" 
acfrs_county_parish_borough_21 <- rbind(acfrs_county_21, louisiana_parish_21) %>% 
...
county_pop_census_acfrs_21 <- acfrs_county_parish_borough_21 %>% 
            left_join(census_county_2020, by = c("state.abb", "name")) %>% 
  
  # bind with Alaska
            rbind(alaska_county_acfrs_census_21) %>% 
            arrange(desc(population)) %>% 
            drop_na(population) 
  
county_pop_census_acfrs_21 %>% write.csv("output/county_gov_21.csv")
...
```{r}
acfrs_geoID_NO_population_21 <- acfrs_NOT_cityplace_21 %>% 
  filter(!id %in% county_pop_census_acfrs_21$id) %>%  # filter out counties
  ...
  # write result
  acfrs_geoID_NO_population_21 %>% write.csv("output/acfrs_geoID_NO_population_21.csv")

  # check how many of these are town
acfrs_geoID_NO_population_21 %>% 
  filter(str_detect(original_name, "(township of)|(town of)"))

  
other_entities_21 <-  acfrs_NOT_cityplace_21 %>% 
  filter(!id %in% county_pop_census_acfrs_21$id) %>%
    filter(!id %in% acfrs_geoID_NO_population_21$id) 
 
 # why so few counties 2021? Use county 2020 to check if some counties are in the remaining other_entities
 
 county_pop_census_acfrs_21 %>% filter(id %in% county_pop_census_acfrs_20$id)
```
